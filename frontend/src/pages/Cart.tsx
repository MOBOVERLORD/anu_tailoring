  ...
  const checkout = async () => {
    ...
    try {
      const order = await api<Order>("/api/orders", { method: "POST", body: JSON.stringify({
        ...
        items: cart.lines.filter((line) => line.kind === "design").map((line) => ({ design_id: line.design.id, measurement_profile_id: line.measurement_profile_id, cloth_source: line.cloth_source, colour_preference: line.colour_preference, reference_design_ids: line.reference_design_ids || [], reference_photo_ids: line.reference_photo_ids || [], fabric_choice: line.fabric_choice, custom_instructions: line.custom_instructions })),
    ...
    <div className="cart-layout">
      <section className="cart-lines"><header><div><Store size={20} /><span><small>Ordering from</small><strong>{cart.vendorName}</strong></span></div><button className="button button-secondary button-small" onClick={cart.clear} type="button"><Trash2 size={15} /> Clear cart</button></header>
        {cart.lines.map((line) => <article className="cart-line" key={line.key}>
          ...
          <div className="cart-line-copy"><small>{line.kind === "design" ? "Tailored design" : "Shop product"}</small><strong>{line.kind === "design" ? line.design.title : line.product.title}</strong>{line.kind === "design" ? <p>{line.measurement_name} · {line.cloth_source === "vendor_supplied" ? "Vendor cloth" : "My cloth"}{line.colour_preference ? ` · ${line.colour_preference}` : ""}{(line.reference_design_ids?.length || line.reference_photo_ids?.length) ? ` · ${(line.reference_design_ids?.length || 0) + (line.reference_photo_ids?.length || 0)} design references` : ""}</p> : <p>{[line.selected_size, line.selected_color].filter(Boolean).join(" · ") || `Sold per ${line.product.unit}`}</p>}</div>
