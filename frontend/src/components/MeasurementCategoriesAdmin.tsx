    ...
    <section className="admin-panel">
      ...
      {editing && <Dialog className="measurement-category-dialog" description="Use one field per line. Standard sizes are optional and prefill the matching fields." onClose={() => setEditing(null)} title={editing === "new" ? "New measurement category" : `Edit ${editing.name}`}>
        <form className="dialog-form form-stack" onSubmit={save}>
          <div className="form-grid"><div className="field"><label htmlFor="category-name">Display name</label><input id="category-name" maxLength={100} minLength={2} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Saree blouse" required value={form.name} /></div><div className="field"><label htmlFor="category-key">Garment key</label><input id="category-key" maxLength={50} onChange={(event) => setForm({ ...form, garment_type: event.target.value })} pattern="[a-zA-Z][a-zA-Z0-9_ ]+" placeholder="saree_blouse" required value={form.garment_type} /></div></div>
          <div className="form-grid"><div className="field"><label htmlFor="category-gender">For</label><AppSelect id="category-gender" onValueChange={(value) => setForm({ ...form, gender: value as MeasurementCategory["gender"] })} options={[{ value: "women", label: "Women" }, { value: "men", label: "Men" }, { value: "unisex", label: "Unisex" }, { value: "kids", label: "Kids" }]} value={form.gender} /></div><div className="field"><label htmlFor="category-order">Display order</label><input id="category-order" max={10_000} min={0} onChange={(event) => setForm({ ...form, sort_order: boundedNumber(event.target.value, 0, 10_000) })} type="number" value={form.sort_order} /></div></div>
          <div className="field"><label htmlFor="category-fields">Measurement fields</label><textarea id="category-fields" maxLength={8000} onChange={(event) => setForm({ ...form, fields: event.target.value })} placeholder={"chest | Chest\nwaist | Waist\nsleeve_length | Sleeve length"} required rows={7} value={form.fields} /><small>Format: field_key | Customer-facing label</small></div>
          <div className="field"><label htmlFor="category-sizes">Standard sizes <small>optional</small></label><textarea id="category-sizes" maxLength={8000} onChange={(event) => setForm({ ...form, sizes: event.target.value })} placeholder={"S: chest=36, waist=30\nM: chest=38, waist=32"} rows={5} value={form.sizes} /><small>One size per line. Every key must exist in the measurement fields above.</small></div>
