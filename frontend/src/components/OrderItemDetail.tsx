    ...
    <div className={`order-item-detail ${order.combined_order ? "is-combined" : ""}`}>
      ...
      <section className="order-detail-grid">
        <div className="order-detail-design">
          ...
          <div><span className={`work-status work-${item.work_status}`}><Clock3 size={13} /> {workLabels[item.work_status]}</span><h3>{item.design.title}</h3><p>{item.design.vendor_name || "Vastrivo"}</p><strong>{item.design.is_custom_request_template && !item.invoice && !order.invoice ? "Tailoring price: awaiting vendor quote" : `Tailoring service: ₹${item.price.toLocaleString("en-IN")}`}</strong>{canVendorReject && <button className="button button-secondary danger-text order-reject-button" disabled={Boolean(busy)} onClick={() => setRejecting(true)} type="button"><XCircle size={16} /> Reject order</button>}</div>
        </div>
