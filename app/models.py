...
class User(Base):
    ...
    # Relationships
    measurement_profiles: Mapped[List["MeasurementProfile"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    addresses: Mapped[List["DeliveryAddress"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    orders: Mapped[List["Order"]] = relationship(back_populates="user")
    liked_designs: Mapped[List["Design"]] = relationship(secondary=user_liked_designs, back_populates="liked_by_users")
...
class MeasurementProfile(Base):
    ...
    profile_name: Mapped[str] = mapped_column(String(50))  # e.g., "Self - Formal", "Dad", "Sister"
    gender: Mapped[str] = mapped_column(String(20))       # "men" / "women" / "unisex"
    unit: Mapped[str] = mapped_column(String(10), default="inches") # "inches" or "cm"
...
class Design(Base):
    ...
    liked_by_users: Mapped[List["User"]] = relationship(secondary=user_liked_designs, back_populates="liked_designs")
...
class Order(Base):
    ...
    address_id: Mapped[int] = mapped_column(ForeignKey("delivery_addresses.id"))
...
class OrderItem(Base):
    ...
    measurement_profile_id: Mapped[int] = mapped_column(ForeignKey("measurement_profiles.id"))
